                                ...
                                @if(!empty($product->addOn) && $product->addOn->count() > 0)
                                    ...
                                    <div class="product-buttons">
                                        ...
                                        @if($is_available == 1 && $product->variant[0]->quantity >= $product->minimum_order_count)
                                            <a href="#" data-toggle="modal" data-target="#addtocart" class="btn btn-solid addToCart {{ (($checkSlot == 0  && $vendor_info->is_vendor_closed == 1) || ($product->has_inventory && $product->variant[0]->quantity <= $product_quantity_in_cart)) ? 'btn-disabled' : '' }}"><i class="ti-shopping-cart"></i> {{__('Add To Carsst')}}</a>
                                        @endif
