            ...
            @foreach ($listData as $key => $data)
                ...
                <div class="col-xl-3 col-md-3 col-6 mt-3">
                    <a href="{{ route('productDetail', [$data->vendor->slug, $data->url_slug]) }}" target="_blank"
                        class="product-box scale-effect mt-0 product-card-box position-relative al_box_third_template al">
                            ...
                            <div class="inner_spacing w-100">
                                    ...
                                    @if ($client_preference_detail)
                                        @if ($client_preference_detail->rating_check == 1)
                                            @if ($data->averageRating > 0)
                                                <span
                                                    class="rating">{{ number_format($data->averageRating, 1, '.', '') }}
                                                    <i class="fa fa-star text-white p-0"></i></span>
                                            @endif
                                        @endif
                                    @endif
                                </h3>
                                <div class="product-description_list border-bottom">
                                    ...
                                    @if (@$data->vendor->is_seller == 1)
                                        <h6 class="sold-by d-flex">
                                            <b> <img class="blur-up lazyload" data-src="{{ $favicon }}"
                                                    alt="{{ $data->vendor->Name }}"
                                                    style="width: 25px !important; height: 25px;"></b> <b> Order by
                                                clickokart </b>
                                        </h6>
                                    @endif
                                </div>
                                @if (($data->inquiry_only == 0) && ($is_service_product_price_from_dispatch_forOnDemand !=1))
                                    @if ($additionalPreference['is_token_currency_enable'])
                                        ...
                                        {{ getInToken($data->variant_price * $data->variant_multiplier) }}
                                    @else
                                        <h4 class="mt-1">
