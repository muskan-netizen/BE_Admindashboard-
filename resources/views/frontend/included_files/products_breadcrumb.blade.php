...
$breadcrumb = '<li class="breadcrumb-item active" aria-current="page">'.(strlen($translation_name) > 26 ? substr($translation_name,0,22)."..." : $translation_name).'</li>';
$translation_name = ($category->translation->first()) ? $category->translation->first()->name : $category->slug;
$breadcrumb = '<li class="breadcrumb-item"><a href="'.route("categoryDetail",$category->slug).'">'.$translation_name.'</a></li>'.$breadcrumb;
...
do{
    if(strtolower($subParent->slug) != "root"){
        ...
        $breadcrumb = '<li class="breadcrumb-item"><a href="'.route("categoryDetail",$subParent->slug).'">'.$translation_name.'</a></li>'.$breadcrumb;
    }
    $subParent = $subParent->allParentsAccount;
} while(!empty($subParent));
$breadcrumb = '<li class="breadcrumb-item"><a href="'.route("userHome").'">Home</a></li>'.$breadcrumb;
@endphp
