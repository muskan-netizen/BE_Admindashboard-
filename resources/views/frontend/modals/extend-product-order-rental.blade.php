        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="end_date">Product End Date</label>
                    <input type="text" name="product_end_date" value="{{$request['vendor_end_date_time']}}" readonly>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label for="end_date">New End Date</label>
                    <input type="text" class="datepicker" name="product_new_end_date" id="product_new_end_date" value="" placeholder="Select Date">
                </div>
            </div>
        </div>
            ...
            <div class="mt-0">{{ __('Duration') }}:
                ...
                <div class="qty-box mb-3">
                    <div class="input-group" style="justify-content: left;">
                        <span class="input-group-prepend">
                            <button type="button" class="btn incremental-left-minus" data-type="minus" data-field=""
                                data-batch_count={{ $product->batch_count }}
                                data-minimum_order_count={{ $product->minimum_order_count }}><i
                                    class="ti-angle-left"></i>
                            </button>
                        </span>
                        <input style="display: none" readonly
                            step="{{ $product->additional_increments * 60 + $product->additional_increments_min }}"
                            type="number" min="0" name="incremental_hrs"
                            onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}"
                            id="incremental_hrs" class="form-control input-qty-number incremental_hrs"
                            value="{{ $product->additional_increments * 60 + $product->additional_increments_min }}">

                        <input type="hidden" min="0" name="total_hrs" id="total_hrs"
                            value="{{ getMinutes($product->minimum_duration, $product->minimum_duration_min) }}">

                        <input style="width: 135px" readonly type="text" id="incremental_hrs_hidden"
                            class="form-control input-qty-number"
                            value="{{ $product->minimum_duration . ' hour ' . $product->minimum_duration_min . ' min' }}">
                        <input type="hidden" name="first_variant" id="first_variant"
                    ...
                    <div class="duration">
                        <div class="total_duration d-flex align-items-center">
                            ...
                            <b class="total_price m-0 pr-1">
                                {{ number_format(@$product->variant[0]->price, 2, '.', ',') }}
                            </b> {{ __('for first') }}
                ...
                <div class="disclaimer mb-3">
                    <span class="d-flex align-items-center">
                        ...
                        <b class="px-1 variant_incremental_price">{{ Session::get('currencySymbol') }}{{ number_format(@$product->variant[0]->incremental_price, 2, '.', ',') }}
                        </b>
                        {{ __('per') }}
                        <b class="px-1 addtional_hrs">{{ $product->additional_increments }}</b> {{ __('hour') }}
                        <b class="px-1 addtional_min">{{ $product->additional_increments_min }} </b>
                        {{ __('min') }}
...
<script type="text/javascript">
    $('document').ready(function(){
        $('#product_new_end_date').datetimepicker({
            format:'m/d/y h:m A',
        });
    });
    ...
    $(function(e) {
        total_min = default_minutes = timeConvertCal('{{ $product->minimum_duration }}',
            '{{ $product->minimum_duration_min }}');
        ...
        default_step = timeConvertCal('{{ $product->additional_increments }}',
            '{{ $product->additional_increments_min }}');
        ...
        $checkoutInput = $('#product_new_end_date');
        $checkinInput.val(moment().format("M/DD/YY hh:mm A"));
        $checkoutInput.val(moment().add(min_dur_hrs, 'hours').add(min_dur_min, 'minutes').format(
            "M/DD/YY hh:mm A"));
        $(".incremental-left-minus").on("click", function() {
            ...
            if (incremental_hrs > 0) {
                $checkoutInput.val(moment(end_current_time).add(incremental_hrs, 'minutes').format(
                    "M/DD/YY hh:mm A"));
                var checkOutPicker = $checkoutInput.data('daterangepicker');
                checkOutPicker.setEndDate(moment(end_current_time).add(incremental_hrs, 'minutes')
                    .format("M/DD/YY hh:mm A"));

                var checkInPicker = $checkinInput.data('daterangepicker');

                checkInPicker.setEndDate(moment(end_current_time).add(incremental_hrs, 'minutes')
                    .format("M/DD/YY hh:mm A"));
                ...
                $checkoutInput.val(moment().add(min_dur_hrs, 'hours').add(min_dur_min, 'minutes')
                    .format("M/DD/YY hh:mm A"));
                var checkOutPicker = $checkoutInput.data('daterangepicker');
                checkOutPicker.setEndDate(moment().add(min_dur_hrs, 'hours').add(min_dur_min, 'minutes')
                    .format("M/DD/YY hh:mm A"));

                var checkInPicker = $checkinInput.data('daterangepicker');

                checkInPicker.setEndDate(moment().add(min_dur_hrs, 'hours').add(min_dur_min, 'minutes')
                    .format("M/DD/YY hh:mm A"));
        ...
        $(".incremental-right-plus").on("click", function() {
            ...
            $checkoutInput.val(moment(end_current_time).add(default_step, 'minutes').format(
                "M/DD/YY hh:mm A"));
            var checkOutPicker = $checkoutInput.data('daterangepicker');

            checkOutPicker.setEndDate(moment(end_current_time).add(default_step, 'minutes').format(
                "M/DD/YY hh:mm A"));

            var checkInPicker = $checkinInput.data('daterangepicker');

            checkInPicker.setEndDate(moment(end_current_time).add(default_step, 'minutes').format(
                "M/DD/YY hh:mm A"));
                    ...
                    if (response.data.success) {
                        ...
                        if (available_product_variant) {
                            ...
                            if (product_variant_data) {
                                var incremental_hrs = document.getElementById('incremental_hrs')
                                    .value;
                                populateProductData(product_variant_data);
                                calculation(incremental_hrs, product_variant_data.incremental_price,
                                    product_variant_data.incremental_price_per_min);
        ...
        function calculation(incremental_min, incremental_price, incremental_price_per_min) {
            var total_additional_price = NumberFormatHelper.formatPrice((incremental_min /
        ...
        function populateProductData(productData) {
            if (productData) {
                ...
                default_minutes = timeConvertCal(productData.product.minimum_duration, productData.product
                    .minimum_duration_min);
                default_step = timeConvertCal(productData.product.additional_increments, productData.product
                    .additional_increments_min)
