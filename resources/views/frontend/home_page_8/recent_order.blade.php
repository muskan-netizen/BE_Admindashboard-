        @php $subtotal_order_price = $total_order_price = $total_tax_order_price = 0; @endphp 
            ...
            <div class="order_detail order_detail_data align-items-top pb-3 card-box no-gutters mb-0"  >
                ...
                <div class="progress-order font-12">
                    @if($order['scheduled_slot']==null)
                        ...
                        <span class="badge badge-success ml-2">{{ __('Scheduled') }}</span> <span class="ml-2">{{__('Your order will arrive by ')}}{{$order['converted_scheduled_date_time']}}</span>
                                @else <span class="ml-2">{{__('Your order will arrive by ')}} {{$vendor['ETA']}}</span>
                        @endif
                    @else <span class="badge badge-success ml-2">{{ __('Scheduled') }}</span> <span class="ml-2">{{__('Your order will arrive by ')}}{{$order['converted_scheduled_date_time']}}, {{ __('Slot')}} : {{$order['scheduled_slot']}}</span>
                    @endif
                </div>
                @endif
                                    ...
                                    <div class="col-md-5 mt-md-0 mt-sm-2">
                                        <ul class="price_box_bottom m-0 p-0">
                                            <li class="d-flex align-items-center justify-content-between">
                                                <label class="m-0">{{__('Product Total')}}</label> <span>{{Session::get('currencySymbol')}}{{number_format((float)$vendor['subtotal_amount'], 2, '.', '')}}</span> </li>
                                            <li class="d-flex align-items-center justify-content-between">
                                                <label class="m-0">{{__('Coupon Discount')}}</label> <span>{{Session::get('currencySymbol')}} {{number_format((float)$vendor['discount_amount'], 2, '.', '');}}</span> </li>
                                            <li class="d-flex align-items-center justify-content-between">
                                                <label class="m-0">{{__('Delivery Fee')}}</label> <span>{{Session::get('currencySymbol')}}{{number_format((float)$vendor['delivery_fee'], 2, '.', '');}}</span> </li>
                                            <li class="grand_total d-flex align-items-center justify-content-between">
                                                ...
                                                @endphp
                                                 <span>{{Session::get('currencySymbol')}}{{number_format((float)$vendor['payable_amount'], 2, '.', '');}}</span> </li>
                                        </ul>
                                    </div>
                                </div>
            </div>
