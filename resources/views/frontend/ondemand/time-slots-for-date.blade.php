...
<div class="booking-timea  radio-btns long-radio">   
    <div class="alCustomHomeServiceRadio SlotItems items">
        ...
        <div class="item  vendor_slot  {{ ($slot['value'] ==@$schedule_slot) ? 'checked_item': ''  }}">
            <div class="radios ">
                <input type="radio" value="{{ $slot['value'] }}" name='booking_time' id='time{{$cart_product_id}}{{$key+1}}'  {{ ($slot['value'] ==@$schedule_slot) ? 'checked': ''  }}  class="{{ ($slot['value'] ==@$schedule_slot) ? 'ondemand_checked': ''  }} "/>          
