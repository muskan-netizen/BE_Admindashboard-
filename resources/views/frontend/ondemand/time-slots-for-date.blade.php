...
<div class="booking-time radio-btns long-radio">
    ...
    <div>
        <div class="radios">
            <!-- <input type="radio" value='{{$date}} - {{@$time_slots[$key+1]}}' name='booking_time{{$cart_product_id}}' id='time{{$cart_product_id}}{{$key+1}}'/> -->
            <input type="radio" value='{{$date}} - {{@$time_slots[$key+1]}}' name='booking_slot[{{$cart_product_id}}][booking_time]' id='time{{$cart_product_id}}{{$key+1}}'/>
