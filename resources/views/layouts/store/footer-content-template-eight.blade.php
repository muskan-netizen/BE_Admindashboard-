...
<style>
    :root {
        ...
        --footer-divider: rgba(148, 163, 184, 0.12);
    }

    .al-footer-redesigned {
    ...
    .al-footer-contact li span,
    .al-footer-contact li a {
        color: var(--footer-link);
        font-size: 14px;
        line-height: 1.6;
        text-decoration: none;
    ...
    .al-footer-contact li a:hover {
        color: var(--footer-link-hover);
    }

    /* App download section */
    .al-footer-app-section {
    ...
    .al-footer-app-section h4 {
        margin-bottom: 20px;
    }

    .al-footer-app-section h4::after {
        display: none;
    }
    ...
    .al-footer-qr-item a svg {
        height: 30px;
        display: block;
    }

    /* Store buttons without QR */
    .al-footer-store-buttons {
        display: flex;
        gap: 12px;
        flex-wrap: wrap;
        justify-content: center;
    ...
    .al-footer-store-buttons a svg {
        height: 36px;
        display: block;
        transition: opacity 0.2s;
    }

    .al-footer-store-buttons a:hover svg {
        opacity: 0.8;
    }
    ...
    .al-footer-payments li {
        margin-bottom: 0 !important;
    }

    .al-footer-payments li a {
        display: flex;
        align-items: center;
        justify-content: center;
        background: var(--footer-surface);
    ...
    .al-footer-payments li a:hover {
        border-color: var(--footer-accent);
        transform: translateY(-1px);
        padding-left: 10px;
    }

    .al-footer-payments li a img {
        height: 22px;
        width: auto;
        display: block;
    }
    ...
    .al-footer-bottom p i {
        margin-right: 4px;
    }

    /* Mobile app buttons on mobile */
    .al-footer-mobile-apps {
        background: var(--footer-surface);
        border-radius: 10px;
        padding: 16px;
        margin-bottom: 24px;
    ...
    .al-footer-mobile-apps .al-footer-qr-grid {
        flex-wrap: wrap;
    }
    ...
    /* Language & Currency modal tweaks */
    .mobile-setting .show-div h6 {
        font-weight: 600;
        color: #334155;
        font-size: 14px;
        text-transform: uppercase;
    ...
    .mobile-setting .show-div ul li {
        list-style: none;
    }

    .mobile-setting .show-div ul li a {
        display: inline-block;
        padding: 6px 14px;
        border-radius: 6px;
    ...
    .mobile-setting .show-div ul li.active a,
    .mobile-setting .show-div ul li a:hover {
        background: var(--footer-accent);
        color: #fff;
        border-color: var(--footer-accent);
    }
...
<footer id="footer" class="al-footer-redesigned al_footer_template_three">
    <div class="al-footer-main">
        <div class="al-footer-grid">
                ...
                <ul class="al-footer-contact">
                    ...
                    <li>
                        ...
                        <a href="tel: {{$clientData ? ($clientData->contact_phone_number ?? $clientData->phone_number) : ''}}">
                            <span>{{$clientData ? ($clientData->contact_phone_number ?? $clientData->phone_number) : ''}}</span>
                        </a>
                    </li>
                    <li>
                        <i class="fa fa-envelope"></i>
                        <a href="mailto: {{$clientData ? ($clientData->contact_email ?? $clientData->email) : ''}}" style="text-transform:none">
                            <span>{{$clientData ? ($clientData->contact_email ?? $clientData->email) : ''}}</span>
                        </a>
                    </li>
                </ul>
            </div>
            ...
            {{-- ===== FIND OUR APP (Desktop) ===== --}}
            @if($client_preference_detail->show_qr_on_footer)
            <div class="al-footer-col d-none d-md-block">
                <div class="al-footer-app-section">
                    <h4>{{ __('Find Our App On Mobile') }}</h4>
                    <div class="al-footer-qr-grid">
                        <div class="al-footer-qr-item">
                            <div class="al_QRcode">{!! QrCode::size(60)->generate($client_preference_detail->ios_link??'#') !!}</div>
                            <a href="{{$client_preference_detail->ios_link??'#'}}" target="_blank"><svg style="height: 30px" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 161 41"><defs><style> .cls-1{stroke:#515251;stroke-miterlimit:10;}.cls-2{fill:#fff;}</style></defs><rect class="cls-1" x="0.5" y="0.5" width="160" height="40" rx="8"/><path class="cls-2" d="M55.83,18.72A5.82,5.82,0,0,0,56.49,29a2,2,0,0,1-.17.59,15.93,15.93,0,0,1-2.92,4.84,3.38,3.38,0,0,1-4.45,1,5.78,5.78,0,0,0-5.07,0,3.09,3.09,0,0,1-3.9-.58c-3.36-3.58-5.07-7.77-4.44-12.73a6.51,6.51,0,0,1,5.38-5.81,5.13,5.13,0,0,1,2.74.28,5.62,5.62,0,0,0,4.87,0,6,6,0,0,1,6.85,1.52A7.21,7.21,0,0,0,55.83,18.72Z" transform="translate(-22.39 -4.28)"/><path class="cls-2" d="M45.92,15.82A5.64,5.64,0,0,1,51,10C51.69,12.67,48.65,16.21,45.92,15.82Z" transform="translate(-22.39 -4.28)"/></svg></a>
            ...
            <div class="al-footer-col">
                ...
                <ul class="al-footer-payments">
                    @foreach($paymentMethod as $payment_method)
                    <li>
                        <a href="#" aria-label="{{ $payment_method->name}}">
                            <img alt="{{ $payment_method->name}}" src="{{ $payment_method->image_url }}">
                        </a>
                    </li>
                    @endforeach
                </ul>
                ...
                <div class="al-footer-social-title">{{ __('Keep In Touch') }}</div>
                <ul class="al-footer-social">
                    @foreach($social_media_details as $social_media_detail)
                    <li>
                        <a href="{{http_check($social_media_detail->url)}}" target="_blank" title="{{$social_media_detail->icon ? ucfirst($social_media_detail->icon) : 'Facebook'}}">
    ...
    <div class="al-footer-bottom">
        <div class="al-footer-bottom-inner">
            @php
            ...
            <p>
                ...
                @if(isset($company_name))
                    @if(!empty($company_name->key_value))
                        | {{$company_name->key_value}}
                    @endif
                @endif
