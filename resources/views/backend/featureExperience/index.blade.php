@extends('layouts.vertical', ['title' => 'Destination'])
@section('css')
...
<div class="container-fluid alCatalogPage">
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('Destination') }}</h4>
            </div>
        </div>
    ...
    <div class="row catalog_box al_catalog_box">
            ...
            <div class="card-box">
                <div class="row" style="max-height: 600px; overflow-x: auto">
                    <div class="col-sm-12 mb-2 d-flex justify-content-between align-items-center">
                        <h4 class=""> {{ __("Destination") }}</h4>
                        <button class="btn btn-info waves-effect waves-light text-sm-right openDestinationModal" dataid="0">
                        ...
                        <div class="row addon-row">
                            <div class="col-md-12">
                                ...
                                <table class="table table-centered table-nowrap table-striped" id="varient-datatable">
                                    ...
                                    <tbody>
                                        @foreach($destination as $set)
                                            <tr>
                                                <td>{{$set->id}}</td>
                                                <td>{{$set->title}}</td>
                                                <td>
                                                    <a class="action-icon editDestination" dataid="{{$set->id}}" href="javascript:void(0);">
                                                        <h3> <i class="mdi mdi-square-edit-outline"></i> </h3>
                                                    </a>

                                                    <a class="action-icon deleteDestination" dataid="{{$set->id}}" href="javascript:void(0);"> <i class="mdi mdi-delete"></i></a>
                                                    <form action="{{route('destination.delete', $set->id)}}" method="POST" style="display: none;" id="DestinationDeleteForm{{$set->id}}">
                                                        @csrf
                                                        @method('DELETE')
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header border-bottom">
                        <h4 class="modal-title">{{ __("Update Destination") }}</h4>
        ...
        <div id="addDestinationmodal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header border-bottom">
                        <h4 class="modal-title">{{ __("Create Destination") }}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <form id="addAddonForm" method="post" enctype="multipart/form-data" action="{{route('destination.store')}}">
                        @csrf
                        <div class="modal-body" id="AddAddonBox">
                            <div class="row">
                                ...
                                <div class="col-6 mb-2">
                                    ...
                                    {!! Form::text('address', '', ['class'=>'form-control', 'id' => 'destination-address', 'required' => 'required']) !!}
                                    {!! Form::hidden('longitude', '', ['class'=>'form-control', 'id' => 'longitude']) !!}
                                    {!! Form::hidden('latitude', '', ['class'=>'form-control', 'id' => 'latitude']) !!}
                                </div>
                                <div class="col-6 mb-2">
