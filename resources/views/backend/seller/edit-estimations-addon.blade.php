<div class="row">
    <div class="col-md-12">
        <div class="row rowYK">
            ...
            <input type="hidden" name="update_id" value="{{Crypt::encrypt($addon->id)}}">
            ...
            <div class="col-md-12" style="overflow-x: auto;">
                <table class="table table-borderless mb-0 optionTableEdit" id="edit_addon-datatable">
                   ...
                   <tr class="input_tr">
                        ...
                        @foreach($languages as $langs)
                            @php $optData = $optDataId = $myIdes = $estimate_addon_option_translations_id = ''; @endphp
                            @foreach($opt->translation as $opt_trans)
                                @if($opt_trans->language_id == $langs->language_id)
                                    @php
                                        $optData = $opt_trans->title;
                                        $estimate_addon_option_translations_id = $opt_trans->id;
                                        $optDataId = $opt_trans->estimate_addon_opt_id;
                                        $myIdes = $opt_trans->id
                                    @endphp
                                @endif
                            @endforeach
                            <td>
                             <input type="hidden" name="estimate_addon_option_translations_id[{{$langs->language_id}}][]" class="form-control" value="{{$estimate_addon_option_translations_id}}">
