                                    ...
                                    @if (isset($order->vendors) &&
                                        ...
                                        <div class='inner-div d-inline-block' style="float: right;">
                                            <form method='POST' action='"+full.destroy_url+"'>
                                                ...
                                                <button type='button' class='btn btn-danger' id="create_dispatch_request"
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-4">
                                            ...
                                            <p>
                                                ...
                                                @endphp
                                                <a href="{{ $order->vendors->first()->dispatch_traking_url }}"
                                ...
                                @elseif(isset($order->vendors) &&
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-4">
                                            <h5 class="mt-0">{{ __('Tracking ID') }}:</h5>
                                            <p>
                                                <a href="{{ $order->vendors->first()->lalamove_tracking_url }}"
                                                    target="_blank">#{{ $order->vendors->first()->web_hook_code }}</a>
                                            </p>
                                        </div>
                                    </div>
                                @endif
                                            ...
                                            @foreach ($order_status_options as $order_status_option)
                                                @if (!in_array($order_status_option->id, $vendor_order_status_option_ids))
                                                    ...
                                                    <li class="{{ $class }} {{ $glow }}  @if (in_array($order_status_option->id, $open_option)) open-for-update-status @else disabled @endif"
                                                        ...
                                                        @endif
                                                        <p class="text-muted"
                                                            id="text_muted_{{ $order_status_option->id }}">
                                                            @if ($date)
                                                                <small
                                                                    class="text-muted">{{ dateTimeInUserTimeZone($date, $timezone) }}</small>
                                                            @endif
                            ...
                            <table class="table table-bordered table-centered mb-0">
                                                ...
                                                <tr>
                                                    <th scope="row" class="product-modal2">
                                                        ...
                                                        <a href={{ $product_url }}"
                                                            ...
                                                            {{ $product->product_name }} @if($product->product->is_long_term_service ==1)   <span class="badge badge-info"> {{ __('Long Term Service') }}</span> @endif
                                                        </a>
                                                        ...
                                                        @foreach ($product->prescription as $pres)
                                                            <br><a target="_blank"
                                                                href="{{ $pres ? @$pres->prescription['proxy_url'] . '74/100' . @$pres->prescription['image_path'] : '' }}">{{ $product->prescription ? 'Prescription' : '' }}</a>
                                                        @endforeach

                                                        <p class="p-0 m-0">
                                                            {{ substr($product->product_variant_sets, 0, -2) }}</p>
                                                        @if ($product->addon && count($product->addon))
                                                            <hr class="my-2">
                                                            <h6 class="m-0 pl-0"><b>{{ __('Add Ons') }}</b></h6>
                                                            @foreach ($product->addon as $addon)
                                                                <p class="p-0 m-0">
                                                                    {{ $addon->option->translation_title }}</p>
                                                            @endforeach
                                                        @endif
                                                    </th>
                                                    <td>
                                ...
                                @endif
                                    ...
                                    @endif
                                        ...
                                        <tr>
                                            <th scope="row" colspan="4" class="text-end">
                                                {{ $client_head->name }} {{ __('Revenue') }} :</th>
                        ...
                        <div class="col-lg-6 card-body">
                            ...
                            @if (isset($order->address) && !empty($order->address->state))
                                <p class="mb-2"><span class="fw-semibold me-2">{{ __('State') }}:</span>
                                    {{ $order->address ? $order->address->state : '' }}</p>
                            @endif
                            <p class="mb-0"><span
                        ...
                        @if (isset($driver_data->name))
                            <div class="col-lg-6 card-body">
                                ...
                                <p class="mb-2"><span class="fw-semibold me-2">{{ __('Name') }}:</span>
                                    {{ $driver_data->name ? $driver_data->name : '' }}</p>
                                <p class="mb-2"><span class="fw-semibold me-2">{{ __('Contact Number') }}:</span>
                                    {{ $driver_data->phone ? $driver_data->phone : '' }}</p>
                            </div>
                        @endif
                                ...
                                <form class="" action="{{ route('order.upload.report') }}" method="post"
                                    ...
                                    <div class="card px-2">
                                        ...
                                        <p class="dropify-error">Ooops, something wrong appended.</p>
                                            ...
                                            <input required type="file" accept="image/*,.pdf,.doc"
                                                data-plugins="dropify" name="file_name" class="dropify"
                                                data-default-file="">
                ...
                <div class="card mb-0 h-100">
                    <div class="card-body">
                        ...
                        <p class="mb-2"><span class="fw-semibold me-2">{{ __('Payment By') }} :</span>
                            {{ $order->paymentOption ? $order->paymentOption->title : '' }}</p>
                        @if ($order->payment)
                            <p class="mb-2"><span class="fw-semibold me-2">{{ __('Transaction Id') }} :</span>
                                {{ $order->payment ? $order->payment->transaction_id : '' }}</p>
                        @endif
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        @if ($order->comment_for_pickup_driver)
                            <p class="mb-2 text-danger"><span
                                    class="fw-semibold me-2">{{ __('Comment for Pickup Driver') }} :</span>
                                {{ $order->comment_for_pickup_driver ?? '' }}</p>
                        @endif

                        @if ($order->comment_for_dropoff_driver)
                            <p class="mb-2 text-danger"><span
                                    class="fw-semibold me-2">{{ __('Comment for Dropoff Driver') }} :</span>
                                {{ $order->comment_for_dropoff_driver ?? '' }}</p>
                        @endif

                        @if ($order->comment_for_vendor)
                            <p class="mb-2 text-danger"><span
                                    class="fw-semibold me-2">{{ __('Comment for Vendor') }} :</span>
                                {{ $order->comment_for_vendor ?? '' }}</p>
                        @endif
                        ...
                        @if ($order->specific_instructions)
                            <p class="mb-2 text-danger"><span
                                    class="fw-semibold me-2">{{ __('Specific instructions') }} :</span>
                                {{ $order->specific_instructions ?? '' }}</p>
                        @endif
