...
<div class="col-md-4 ">
    <div class="form-group d-flex justify-content-between mb-3 alCustomToggleColor">
        <label for="is_vendor_marg_configuration" class="mr-2 mb-0">{{ __('SMS for Complete, cancel and booked ride') }}<small
                class="d-block pr-5">{{ __('Enable sms for Complete, cancel and booked ride.') }}</small></label>
        <span> <input type="checkbox" data-plugin="switchery" name="is_sms_complete_cancel_book_login_ride_enable"
                id="is_sms_complete_cancel_book_login_ride_enable_switch" class="form-control checkbox_change"
                data-className="is_sms_complete_cancel_book_login_ride_enable" data-color="#43bee1"
                @if (@$getAdditionalPreference['is_sms_complete_cancel_book_login_ride_enable']) checked='checked' @endif>
        </span>
        <input type="hidden" @if (@$getAdditionalPreference['is_sms_complete_cancel_book_login_ride_enable'] == 1) value="1" @else value="0" @endif
            name="is_sms_complete_cancel_book_login_ride_enable" id="is_sms_complete_cancel_book_login_ride_enable" />
    </div>
</div>
