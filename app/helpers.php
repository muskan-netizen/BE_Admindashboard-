...
if (!function_exists('getAdditionalPreference')) {    
    ...
    function getAdditionalPreference($key=array()){
        ...
        $dbreturn= [];
        if(sizeof($key)){
            $result = (checkColumnExists('client_preference_additional','key_name')) ? ClientPreferenceAdditional::select('key_name','key_value')->whereIn('key_name',$key)->where(['client_code' => $user->code])->get()->toArray() : [];
            $return = array_column($result, 'key_value', 'key_name');
                if(sizeof($result)){
