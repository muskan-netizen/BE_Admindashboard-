...
class DropoffSlotController extends Controller
{
    public function store(Request $request, $domain = '', $id)
    {
        ...
        if($request->dropoff_stot_type == 'day'){
            ...
            $slot->start_time   = $request->dropoff_start_time;
            $slot->end_time     = $request->dropoff_end_time;
            ...
            foreach ($request->dropoff_week_day as $key => $value) {
            ...
            $slotData['start_time']         = $request->dropoff_start_time;
            $slotData['end_time']           = $request->dropoff_end_time;
            $slotData['specific_date']      = $request->dropoff_slot_date;
        ...
        if($request->dropoff_edit_type == 'day') {
            ...
            if(!$slotDay){
                ...
                if($request->slot_type_edit == 'date'){
                    ...
                    $dateSlot->start_time       = $request->dropoff_start_time;
                    $dateSlot->end_time         = $request->dropoff_end_time;
                    $dateSlot->specific_date    = $request->dropoff_slot_date;
            ...
            $slotDay->day = $request->dropoff_edit_day;
            $slotDay->save();

        }else{
            $dateSlot = VendorSlotDate::where('id', $request->dropoff_edit_type_id)->first();
            if(!$dateSlot){
                $dateSlot = new VendorSlotDate();
            }else{
                if( $request->dropoff_slot_type_edit == 'day' ){
                    ...
                    })->where('day', $request->dropoff_edit_day)->delete();
                    ...
                    $sday->day = $request->dropoff_edit_day;
                    $sday->save();
                    return redirect()->back()->with('success', 'Slot saved successfully!');
                }
            }
    ...
    {
        if($request->dropoff_slot_type == 'date'){
            if($request->dropoff_old_slot_type == 'day')
            {
                VendorSlotDate::updateOrCreate([
                    'vendor_id'=>$id,
                    'specific_date' => $request->dropoff_slot_date
                ],[
                    'working_today' => 0
                ]);
            }else{
                $dateSlot = VendorSlotDate::where('id', $request->dropoff_slot_id)->delete();
            }
        } else {
            $slotDay = SlotDay::where('slot_id', $request->dropoff_slot_id)->get();
            if($slotDay->count() == 1){
                $vendorSlot = VendorSlot::where('id', $request->pickup_slot_id)->where('slot_type', '2')->delete();
            }
            $slot_day = SlotDay::where('id', $request->dropoff_slot_day_id)->delete();
        }
    ...
    public function returnJson(Request $request, $domain = '', $id)
    {
        ...
        $lst = count($date) - 1;
        $slot = VendorSlot::join('slot_days', 'slot_days.slot_id', 'vendor_slots.id')->where('vendor_id', $id)->orderBy('slot_days.day', 'asc')->where('slot_type', '2')->get();
