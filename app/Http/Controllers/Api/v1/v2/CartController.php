            ...
            if ($isChefProduct) {
                $existingChefInCart = CartProduct::where('cart_id', $cart_detail->id)
                    ->whereHas('product.category.categoryDetail', function ($q) use ($CHEF_CATEGORY_ID) {
                        $q->whereIn('id', $CHEF_CATEGORY_ID);
                    })
                    ->exists();

                if ($existingChefInCart) {
                    return $this->errorResponse(['error' => __('Only one service can be added at a time.'), 'alert' => '1'], 400);
                }
            }
    ...
    public function getCart($cart, $langId = '1', $currency = '1', $type = 'delivery', $code = 'D')
    {
        ...
        try {
            ...
            $cartData = CartProduct::with([
                'vendor',
                'coupon' => function ($qry) use ($cartID) {
    ...
    public function updateSchedule(Request $request, $domain = '')
    {
        ...
        try {
            ...
            if ($user) {
                ...
                Cart::where('status', '0')->where('user_id', $user->id)->update([
