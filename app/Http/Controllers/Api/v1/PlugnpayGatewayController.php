...
class PlugnpayGatewayController extends BaseController
{
    ...
    {
        $plug_pay_creds = PaymentOption::select('credentials', 'test_mode')->where('code', 'khalti')->where('status', 1)->first();
        $creds_arr = json_decode($plug_pay_creds->credentials);
        $api_key = (isset($creds_arr->api_key)) ? $creds_arr->api_key : '';
        $api_secret_key = (isset($creds_arr->api_secret_key)) ? $creds_arr->api_secret_key : '';
        $this->test_mode = (isset($plug_pay_creds->test_mode) && ($plug_pay_creds->test_mode == '1')) ? true : false;
