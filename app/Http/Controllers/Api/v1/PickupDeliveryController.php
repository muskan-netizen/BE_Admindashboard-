    ...
    */
    ...
    public function updatePickupDeliveryOrderByCustomer(Request $request){
        try {
            ...
            $ordervendordata = OrderVendor::where('order_id', $orderdata->id)->first();
            $vendor_id = $ordervendordata->vendor_id;
            $ordervendorproductdata = OrderVendorProduct::where('order_vendor_id', $ordervendordata->id)->select('product_id')->first();
            $tasks = OrderLocations::where('order_id', $orderdata->id)->select('tasks')->first();
            if(isset($request->tasks))
            {
                if(!empty($tasks))
                {
                    OrderLocations::where('order_id', $orderdata->id)->update(['tasks' => $request->tasks]);
                }else{
                    OrderLocations::insert(['tasks' => $request->tasks, 'order_id' => $orderdata->id, 'vendor_id' => $vendor_id, 'product_id' => $ordervendorproductdata->product_id]);
                }
            }
            $request->request->add(['product_id', $ordervendorproductdata->product_id]);
            $request->request->add(['tasks', json_decode($request->tasks)]);
            $request_to_dispatch = $this->placeRequestToDispatch($request, $orderdata, $vendor_id);
            if($request_to_dispatch && isset($request_to_dispatch['task_id']) && $request_to_dispatch['task_id'] > 0){
                $user = Auth::user();
                $order_place['data']['dispatch_traking_url'] = $request_to_dispatch['dispatch_traking_url'];
                $order_place['data']['user_name'] = $user->email;
