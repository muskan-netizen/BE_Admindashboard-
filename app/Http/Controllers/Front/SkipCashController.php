...
class SkipCashController extends Controller
{
    ...
    function generateAuthToken($keyId, $keySecret, $clientID) {
        $data = array(
            'kid' => $keyId,
            'cid' => $clientID,
            'iat' => time(),
            'exp' => time() + 600 // token expiry time, in seconds
        );
        $dataJson = json_encode($data);
        $payload = base64_encode($dataJson);
        $signature = base64_encode(hash_hmac('sha256', $payload, $keySecret, true));
