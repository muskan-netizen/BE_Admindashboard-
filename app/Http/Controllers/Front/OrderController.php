...
class OrderController extends FrontController
{
    ...
    public function orderSave($request, $paymentStatus)
    {
        try {
            ...
            $additionalPreferences = (object) getAdditionalPreference([
                'is_tax_price_inclusive'
            ]);
            ...
            if (getAdditionalPreference([
                'is_gift_card'
            ])['is_gift_card'] == 1 && checkColumnExists('carts', 'gift_card_id')) {
                ...
                if (isset($cart->giftCard) && ! empty($cart->giftCard)) {
                    ...
                    $giftcard = UserGiftCard::with('giftCard')->whereHas('giftCard', function ($query) use ($nowDate) {
                        ...
                        ->where([
                        'is_used' => '0',
                        'gift_card_code' => $cart->user_gift_code
                    ])
                        ->first();
            ...
            if (isset($cart->editingOrder) && ! empty($cart->editingOrder)) {
                ...
                $VendorOrderStatus = VendorOrderStatus::where('order_id', $order->id)->whereNotIn('order_status_option_id', [
                    1,
                    2
                ])->count();
            ...
            if (getAdditionalPreference([
                'is_gift_card'
            ])['is_gift_card'] == 1 && checkColumnExists('orders', 'gift_card_id')) {
