...
class ProductBookingController extends FrontController
{
    ...
    public function checkProductAvailibility(Request $request)
    {
       try {
          ...
          if($variant_product_detail->quantity > $ProductBooking){
            $product_variant_data =  ProductVariant::where('id',$product_variant_id)->with(['product','checkIfInCart'])->first();
            $available_product_variant = $product_variant_id;
          }
          // dd($available_product_variant);
