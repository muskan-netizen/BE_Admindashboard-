            ...
            try {
                ...
                foreach ( $this->data as $row) {
                    ...
                    if (isset($row[0]) && $row[0] != "SKU") { // data of excel check
                        ...
                        // $checker = 1;
                        // }
                        if (isset($row[4]) && $row[4] == "") { // check if category is empty
                            $error[] = "Row " . $i . " : Category cannot be empty";
                            $checker = 1;
                        }
                        if (isset($row[4]) && $row[4] != "") {
                            $category = $row[4];
                            $vendorCategoryExists = VendorCategory::with('category.translation')->whereHas('category.translation', function ($q) use ($category) {
