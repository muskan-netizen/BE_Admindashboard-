...
var fpxBank = '';
var idealBank = {};
$(document).on('change', '#wallet_payment_methods_pending input[name="wallet_payment_method"]', function() {
        alert();
        $('#wallet_payment_methods_error_pending').html('');
        var method = $(this).val();
        var code = method.replace('radio-', '');
        if (code != '') {
            $("#wallet_payment_methods_pending .option-wrapper").addClass('d-none');
            $("#wallet_payment_methods_pending ."+code+"_element_wrapper").removeClass('d-none');
        } else {
            $("#wallet_payment_methods_pending .option-wrapper").addClass('d-none');
        }
